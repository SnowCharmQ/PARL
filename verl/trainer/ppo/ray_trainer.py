...
class RayPPOTrainer:
    ...
    def _save_best_checkpoint(self):
        ...
        # path: given_path + `best_checkpoint` + `/actor`
        local_best_checkpoint_folder = os.path.join(
            self.config.trainer.default_local_dir, "best_checkpoint"
        )

        if os.path.exists(local_best_checkpoint_folder):
            shutil.rmtree(local_best_checkpoint_folder)
            print(f"Removed previous best checkpoint at {local_best_checkpoint_folder}")

        actor_local_path = os.path.join(local_best_checkpoint_folder, "actor")
        ...
        if self.use_critic:
            critic_local_path = os.path.join(local_best_checkpoint_folder, str(Role.Critic))
            self.critic_wg.save_checkpoint(
                critic_local_path, None, self.global_steps, max_ckpt_to_keep=max_critic_ckpt_to_keep
            )

        # save dataloader
        local_mkdir_safe(local_best_checkpoint_folder)
        dataloader_local_path = os.path.join(local_best_checkpoint_folder, "data.pt")
        dataloader_state_dict = self.train_dataloader.state_dict()
        torch.save(dataloader_state_dict, dataloader_local_path)

        # latest checkpointed iteration tracker (for atomic usage)
        local_latest_checkpointed_iteration = os.path.join(
            local_best_checkpoint_folder, "best_checkpoint.txt"
        )
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if data_metrics['critic/score/mean'] > self.best_score:
                    self.best_score = data_metrics['critic/score/mean']
