...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    extra_data = batch.non_tensor_batch
    sub_scores_lst = [
        'rouge1', 
        'meteor', 
        'bertscore', 
    ...
    for sub_score in sub_scores_lst:
        if sub_score in extra_data:
            sub_score_data = extra_data[sub_score]
            sub_score_data = torch.tensor(sub_score_data, dtype=torch.float32)
            sub_score_mean = torch.mean(sub_score_data).detach().item()
            sub_score_max = torch.max(sub_score_data).detach().item()
            sub_score_min = torch.min(sub_score_data).detach().item()

            metrics[f"critic/{sub_score}/mean"] = sub_score_mean
            metrics[f"critic/{sub_score}/max"] = sub_score_max
            metrics[f"critic/{sub_score}/min"] = sub_score_min
